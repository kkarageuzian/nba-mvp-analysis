...
```{r}
# Create table for MVP voting results for players who received votes
mvp_votes <- tibble(
  name = c(
    "Joel Embiid",
    "Nikola Jokic",
...
# Merge MVP voting results with custom MVP model dataset
...
```{r}
# Plot for Points + Rebounds + Assists vs MVP Voting Points
ggplot(nba_total %>% filter(mvp_points > 0),
       aes(x = PTS + TRB + AST, y = mvp_points)) +
  geom_point(size = 3) +
  labs(
    title = "Points + Rebounds + Assists vs MVP Voting Points",
    x = "Total Production (PTS + REB + AST)",
    y = "MVP Points"
  ) +
  theme_minimal()
```
